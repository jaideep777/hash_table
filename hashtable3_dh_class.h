...
class HashTable{
	...
	HashFindResult hash_find(Key key){
		...
		while(1) { 	// probe until slot is filled AND the key matches. (i.e., dont stop at empty slot)
			...
			// DEBUG_HT std::cout << id;
	//		// DEBUG_HT if (id == res.firstEmptySlot) std::cout << "__";
			// DEBUG_HT std::cout << ", ";
		...
		// DEBUG ONLY
		// DEBUG_HT if (id != size_t(-1)) std::cout << "|   <" << key << ", " << ht[id].value << "> [" << res.attempts << " tries]" << ((key != ht[id].value*10)? " * FAIL *":"") << std::endl;
		// DEBUG_HT else std::cout << "|   <" << key << ", " << "- " << "> [" << res.attempts << " tries]" << std::endl;

	//	// DEBUG_HT std::cout << "RES (find): " << (int)res.id << " " << res.attempts << std::endl;
	...
	int hash_insert(Key key, Value value, int* duplicate=NULL){
		// DEBUG_HT std::cout << "Insert: <" << key << ", " << value << "> : trying "; std::cout.flush();
		...
		do { 	
			...
			// DEBUG_HT std::cout << id;
			// DEBUG_HT if (id == firstEmptySlot) std::cout << "__";
			// DEBUG_HT std::cout << ", ";
		...
		if (ht[id].key == key && !ht[id].isEmpty){	
			// DEBUG_HT std::cout << "(count = " << count << ") --> Replacing @ [" << id << "]: <" << key << "," << ht[id].value << "> ==> <" << key << "," << value << ">" << std::endl;
			ht[id].value = value; 
		...
		// DEBUG_HT std::cout << " --> stored @ [" << id << "]" << std::endl;
